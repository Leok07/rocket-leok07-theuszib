...
export function CareerStatsBox({
  ...
  if (isLoading || !careerData) {
    return (
      <div className="p-5 rounded-2xl bg-[#09090b] border border-[#1e1e24] animate-pulse space-y-4">
  ...
  const metrics = [
    ...
    },
    {
      ...
      p1Raw: p1Mvps,
      p2Raw: p2Mvps,
      p1Sub: `${p1MvpRate.toFixed(1)}% das vitórias`,
      p2Sub: `${p2MvpRate.toFixed(1)}% das vitórias`,
      icon: Award,
    ...
    <div className="rounded-2xl bg-[#09090b] border border-[#1e1e24] p-4 sm:p-6 space-y-5 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#1e1e24] pb-4">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-[#0e1726] border border-sky-800/60 text-sky-400 shrink-0">
            <History className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-sm sm:text-base font-bold uppercase tracking-wider text-zinc-100 flex items-center gap-2">
              Estatísticas Acumuladas no Ballchasing
            </h3>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span
            className="text-[11px] uppercase font-bold tracking-wider px-3 py-1 rounded-md bg-[#0e1726] border border-sky-600/60 text-sky-300 shadow-[0_0_12px_rgba(56,189,248,0.12)]"
            title="Estatísticas 100% reais extraídas diretamente dos arquivos de replay do Ballchasing."
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Player 1 Card */}
        <div className="p-4 rounded-xl bg-[#0c0c10] border border-sky-900/40 flex items-center justify-between shadow-md">
        ...
        {/* Player 2 Card */}
        <div className="p-4 rounded-xl bg-[#0c0c10] border border-orange-900/40 flex items-center justify-between shadow-md">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {metrics.map((m) => {
          ...
          return (
            <div
              key={m.label}
              className="p-3.5 rounded-xl bg-[#0c0c10] border border-[#1e1e24] space-y-2 hover:border-[#38bdf8]/40 transition-colors"
            >
      ...
      {/* Sinergia de Dupla vs Média Geral (Desktop 2-Column Split) */}
      <div className="p-4 rounded-xl bg-gradient-to-br from-[#0e0e12] via-[#09090b] to-[#0e0e12] border border-[#1e1e24] space-y-3">
        <div className="flex items-center gap-2">
          <Flame className="w-4 h-4 text-amber-400" />
          <h4 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-zinc-200">
            Sinergia na Dupla vs Média dos Replays
          </h4>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          {/* P1 Synergy Delta */}
          <div className="p-3 rounded-lg bg-[#070709] border border-[#1e1e24] space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Produzindo{' '}
              <strong className="text-zinc-100 font-bold">{p1RecentG.toFixed(2)} gols/jogo</strong> na sessão vs{' '}
              <strong className="text-zinc-300">{p1GoalsPerMatch.toFixed(2)} na média dos replays</strong>.
            </p>
          </div>
          ...
          <div className="p-3 rounded-lg bg-[#070709] border border-[#1e1e24] space-y-1.5">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Garantindo{' '}
              <strong className="text-zinc-100 font-bold">{p2RecentSv.toFixed(2)} saves/jogo</strong> na sessão vs{' '}
              <strong className="text-zinc-300">{p2SavesPerMatch.toFixed(2)} na média dos replays</strong>.
            </p>
          </div>
        </div>
      </div>

      {/* Badges e Marcos de Legado - 100% Dinâmicos e Reais */}
      <div className="border-t border-[#1e1e24] pt-4 space-y-2.5">
        <div className="flex items-center gap-2">
          <Medal className="w-4 h-4 text-yellow-400" />
          <h4 className="text-xs sm:text-sm font-bold uppercase tracking-wider text-zinc-300">
            Lideranças Reais Registradas
          </h4>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-[#0c0c10] border border-amber-500/30 text-center space-y-1">
          ...
          <div className="p-3 rounded-xl bg-[#0c0c10] border border-cyan-500/30 text-center space-y-1">
          ...
          <div className="p-3 rounded-xl bg-[#0c0c10] border border-sky-500/30 text-center space-y-1">
            <span className="text-xs text-sky-400 font-black uppercase block tracking-wider">Mais Vitórias</span>
            <span className="font-bold text-zinc-100">{topWinLeader} ({topWinCount.toLocaleString('pt-BR')} Vitórias)</span>
          </div>

          <div className="p-3 rounded-xl bg-[#0c0c10] border border-purple-500/30 text-center space-y-1">
            <span className="text-xs text-purple-400 font-black uppercase block tracking-wider">Top Garçom</span>
